...
int main(void)
{
  ...
  memoryLayout.srcDistribAxes = {{0}};
  memoryLayout.srcAxesOrder   = {{0, 1}};
  memoryLayout.dstDistribAxes = {{1}};
  memoryLayout.dstAxesOrder   = {{1, 0}};

  {
    ...
    const std::size_t srcElemCount = plan.getSrcElemCount().front(); // get the number of elements in the source vector
    const std::size_t dstElemCount = plan.getDstElemCount().front(); // get the number of elements in the destination vector
