classdef TestIdct < afft.test.unit.AbstractTestTransform
  ...
  methods (Static)
    function dstRef = computeReference(src, dim, normalization, dctType)
      ...
      if strcmp(normalization, 'none')
        if isempty(dctType)
          dstRef = matlab.internal.math.transform.mlidct(gather(src), [], dim) * transformSize;
        else
          dstRef = matlab.internal.math.transform.mlidct(gather(src), [], dim, 'Variant', dctType) * transformSize;
        end
      elseif strcmp(normalization, 'unitary')
        if isempty(dctType)
          dstRef = matlab.internal.math.transform.mlidct(gather(src), [], dim);
        else
          dstRef = matlab.internal.math.transform.mlidct(gather(src), [], dim, 'Variant', dctType);
        end
      elseif strcmp(normalization, 'orthogonal')
        if isempty(dctType)
          dstRef = idct(src, [], dim);
        else
          dstRef = idct(src, [], dim, Type=dctType);
        end
      else
  ...
  methods
    function testSuccess(testCase, backend, precision, normalization, gridSize, dctType, dim)
      ...
      dstRef = afft.test.unit.TestIdct.computeReference(src, dim, normalization, dctType);
