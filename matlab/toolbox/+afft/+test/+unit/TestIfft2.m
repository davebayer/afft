classdef TestIfft2 < afft.test.unit.AbstractTestTransform
  ...
  methods (Static)
    ...
    function dstRef = computeReference(src, normalization, symmetricFlag)
      % Compute the reference using the built-in ifft2 function.
      dstRef = ifft2(src, symmetricFlag);
  ...
  methods
    function testSuccess(testCase, backend, precision, normalization, gridSize, symmetricFlag)
      src = afft.test.unit.TestIfft2.generateSrcArray(backend, gridSize, precision, symmetricFlag);

      dstRef = afft.test.unit.TestIfft2.computeReference(src, normalization, symmetricFlag);
      dst    = afft.ifft2(src, ...
                          'backend',       backend, ...
    ...
    function testFailure(testCase, backend, precision, normalization, gridSize, symmetricFlag)
      ...
      try
        dst = afft.ifft2(src, ...
                         ...
                         'normalization', normalization, ...
                         'threadLimit',   afft.test.unit.AbstractTestTransform.cpuThreadLimit);
        testCase.verifyFail('Expected afft.ifft2 to fail');
      catch
      end
    end
  end
